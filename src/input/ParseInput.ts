...
import { getPolicyIdentifier, getRequestIdentifier, getRequestIdentifierOld } from "../test_cases/ODRLUtil";
import { getTestCaseIdentifier } from "../test_cases/TestCaseUtil";
import {getStateOfTheWorldOld, getStateOfTheWorldIdentifierOld, getStateOfTheWorldIdentifier, getStateOfTheWorld} from "../test_cases/SotwUtil";
 ...
 * @returns
 */
export async function parseRequestsOld(requestsDir: string): Promise<Map<string, Policy>> {
    const requests: Map<string, Policy> = new Map<string, Policy>();
    for (const file of getAbsoluteFilePaths(requestsDir)) {
        const store = await fileAsStore(file);
        let requestID: string | undefined
        try {
            requestID = getRequestIdentifierOld(store.getQuads(null, null, null, null));
        } catch {
...
export async function parseStateOfTheWorldOld(stateOfTheWorldDir: string): Promise<Map<string, StateOfTheWorld>> {
    ...
    for (const file of getAbsoluteFilePaths(stateOfTheWorldDir)) {
        ...
        try {
            stateOfTheWorldID = getStateOfTheWorldIdentifierOld(store.getQuads(null, null, null, null));
        } catch {
        ...
        stateOfTheWorldMap.set(stateOfTheWorldID, getStateOfTheWorldOld(store, stateOfTheWorldID, file));
    }
